...
public class 地圖二右控制 : MonoBehaviour
{
    //地圖二右控制
    ...
    void Update()
    {
        ...
        丟東西();
        //攻擊
        if (Input.GetKeyDown(KeyCode.RightControl) && 是否拿起針 == false && 是否拿起AED == false && 是否拿起鋸子 == false && 是否拿起繃帶 == false && 是否拿起實驗體1 == false && 是否拿起實驗熊 == false && 是否拿起實驗輪椅 == false)
        {
    ...
    private void OnTriggerStay(Collider col)
    {
        ...
        if (col.tag == "箱子" && Input.GetKey(KeyCode.RightShift) && 拾取計數器 == 1 && 是否拿起針 == false)
        {
