...
public class 輪椅怪 : MonoBehaviour
{
    ...
    void AIBack()
    {
        Debug.Log("0777777777777777777");
        動畫控制器.SetBool("死亡", false);
        
        Instantiate(生成斷腿模式, this.gameObject.transform.position, this.gameObject.transform.rotation);
        Destroy(this.gameObject);
    }

    void Update()
    {
        if (動畫控制器.GetCurrentAnimatorStateInfo(0).IsName("攻擊") || 動畫控制器.GetCurrentAnimatorStateInfo(0).IsName("死亡")|| 動畫控制器.GetCurrentAnimatorStateInfo(0).IsName("正在倒下"))
        {
