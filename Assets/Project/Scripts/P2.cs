...
public class P2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //攻擊
        if (Input.GetKeyDown(KeyCode.RightControl) && 是否拿起敵人 == false && 是否拿起大蒜 == false)
        {
            if (!攻擊聲.isPlaying)
            {
                攻擊聲.Play();
    ...
    private void OnTriggerStay(Collider col)
    {
        ...
        //祭壇
        if (col.tag == "祭壇" && Input.GetKey(KeyCode.RightShift) && col.GetComponent<祭壇>().顯示吸血鬼 == false && 是否拿起敵人 == true && 拾取計數器 == 0)
        {
            if (!丟.isPlaying)
            {
                丟.Play();
